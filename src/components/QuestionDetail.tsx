...
const client = new GraphQLClient('http://localhost:8080/graphql'); // Replace with your backend URL

export const QuestionDetail = ({ currentState, refetchQuestions, setAnswerLoadingState }) => {
  ...
  const handleCommitQuestion = async () => {
    try {
      ...
      if (currentState.question) {
        ...
        const variables = { id: currentState.question.id, questionText: questionText };
        response = await client.request(UPDATE_QUESTION, variables);
        result = response.updateQuestion;
      } else {
        // Create new question
        const variables = { questionText: questionText, categoryId: currentState.category.id };
        response = await client.request(CREATE_QUESTION, variables);
  ...
  const isCommitDisabled = questionText.trim() === '' || questionText === currentState.question?.questionText; // Disable if question text is empty or unchanged
  ...
  return (
    ...
    >
      <TextareaAutosize 
        minRows={30}
        placeholder={isNewDisabled ? 'Задайте новый вопрос категории '+ currentState.category?.name: 'Отредактируйте вопрос'}
        value={questionText} 
      ...
      <Box sx={{ display: 'flex', width: '100%', justifyContent: 'space-between' }}>
        ...
        <Box sx={{ display: 'flex', gap: '16px' }}>
          <IconButton 
            ...
            sx={{ marginTop: '16px' }}
            color="default"
            disabled={!currentState?.question || questionText === currentState.question.questionText} // Disable if the text is already the original
          >
            <UndoIcon />
          </IconButton>
          <IconButton 
